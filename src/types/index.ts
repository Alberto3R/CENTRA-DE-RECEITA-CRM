...
export interface Contact {
  ...
  user_id: string;
  account_id: string;
  /** Nullable desde a migration 066: contatos de Instagram não têm
   *  telefone (a identidade é o `instagram_id`). */
  phone: string;
  /** Digits-only form of `phone`, generated by the DB (migration 022)
   *  and unique per account. Read-only. */
  phone_normalized?: string;
  /** WhatsApp username id (BSUID), quando o lead chega por username. */
  wa_user_id?: string | null;
  /** IGSID — identidade do contato no canal Instagram (migration 066). */
  instagram_id?: string | null;
  /** @handle do Instagram (enriquecido via Graph). Exibido no lugar do telefone. */
...
export interface Conversation {
  ...
  ai_handoff?: boolean;
  /**
   * Canal (whatsapp_config.id) em que a conversa acontece — WhatsApp ou
   * Instagram (migration 056/066). Null = usar o canal primário. Resolvido
   * no envio por `resolveChannelConfig`.
   */
