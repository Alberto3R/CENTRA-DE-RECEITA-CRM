...
const ROLE_DESCRIPTIONS: Record<InviteRole, string> = {
  ...
  agent:
    'Pode usar a caixa de entrada, contatos, disparos, automações e fluxos. Sem acesso às configurações ou aos membros.',
...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'nossa conta na Central de Receita',
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    const accountName = result?.accountName ?? 'nossa conta na Central de Receita';
    const message = `Entre em ${accountName} na Central de Receita usando este link (válido por ${result?.expiresInDays} dias): ${url}`;
          ...
          <>
            <DialogHeader>
              ...
              <DialogDescription className="text-muted-foreground">
                Compartilhe este link com seu novo colega de equipe. Ele
                poderá criar conta (ou entrar) e ingressar na conta como{' '}
            ...
            <div className="space-y-3 py-2">
              ...
              <div className="rounded-md border border-amber-500/50 bg-amber-500/15 px-3 py-2 text-xs text-amber-200">
                ...
                Nunca armazenamos o link em texto puro — assim que você fechar
                esta janela, ele desaparece. Para compartilhar de novo, revogue
          ...
          <>
            <DialogHeader>
              ...
              <DialogDescription className="text-muted-foreground">
                ...
                WhatsApp, Slack ou qualquer canal que preferir — sem precisar
