...
export interface BuilderState {
  ...
  /** Ensaio: roda sem enviar, registrando o que teria mandado. */
  shadow_mode: boolean;
...
export function FlowEditorProvider({
  ...
  const [state, setStateRaw] = useState<BuilderState>(() => ({
    ...
    shadow_mode: initialFlow.shadow_mode ?? false,
  ...
  const deleteFlow = useCallback(async () => {
    const yes = window.confirm(
      `Excluir "${state.name}"? Todas as execuções ativas terminam imediatamente. Isto não pode ser desfeito.`,
    );
    if (!yes) return;
    try {
      const res = await fetch(`/api/flows/${initialFlow.id}`, {
